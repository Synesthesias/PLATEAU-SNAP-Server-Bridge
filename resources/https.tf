...
# HTTP Listener - Direct routing (for initial deployment)
...
# HTTP Listener Rule for API subdomain
...
# HTTP Listener - Redirect to HTTPS (for later use)
# resource "aws_lb_listener" "http" {
#   load_balancer_arn = aws_lb.default.arn
#   port              = "80"
#   protocol          = "HTTP"
...
# HTTP Listener Rule for CMS subdomain
# Note: Cognito authentication is only supported on HTTPS listeners
# For initial HTTP deployment, authentication is disabled
resource "aws_lb_listener_rule" "cms_http" {
  listener_arn = aws_lb_listener.http.arn
  priority     = 101
