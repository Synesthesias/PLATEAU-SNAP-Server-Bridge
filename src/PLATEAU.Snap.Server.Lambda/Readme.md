...
WORKDIR /src
COPY ["PLATEAU.Snap.Server.Lambda.csproj", "PLATEAU.Snap.Server.Lambda/"]
RUN dotnet restore "PLATEAU.Snap.Server.Lambda/PLATEAU.Snap.Server.Lambda.csproj"

WORKDIR "/src/PLATEAU.Snap.Server.Lambda"
COPY . .
RUN dotnet build "PLATEAU.Snap.Server.Lambda.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "PLATEAU.Snap.Server.Lambda.csproj" \
...
Execute unit tests
```
cd "PLATEAU.Snap.Server.Lambda/test/PLATEAU.Snap.Server.Lambda.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "PLATEAU.Snap.Server.Lambda/src/PLATEAU.Snap.Server.Lambda"
